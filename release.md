...
This guide outlines the steps required to build, tag, and publish new versions of Time Guardian. It covers building the `.exe` and `.msi` for 64-bit platforms (and standard cross-compilation for 32-bit if configured), as well as pushing to GitHub Releases using the GitHub CLI (`gh`).
...
npm run tauri build
```
*(The output artifacts will be placed in `src-tauri/target/release/bundle/msi` and `src-tauri/target/release/bundle/nsis`.)*
...
gh release create v0.0.1 \
  --title "Time Guardian v0.0.1" \
  --notes-file CHANGELOG.md \
  "src-tauri/target/release/bundle/msi/time-guardian_0.0.1_x64_en-US.msi" \
  "src-tauri/target/release/bundle/nsis/time-guardian_0.0.1_x64-setup.exe"
```
